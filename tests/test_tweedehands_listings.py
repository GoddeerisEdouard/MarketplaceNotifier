...
class TestTweedehandsNotifier(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_query_returns_at_least_one_listing(self):
        ...
        async with aiohttp.ClientSession() as cs:
            non_ad_listings = await self.tn._fetch_listings_of_request_url(cs, request_url)
    ...
    async def test_amount_of_listings_should_be_lower_with_location_filter(self):
        ...
        async with aiohttp.ClientSession() as cs:
            ...
            non_ad_listings_without_location_filter = await self.tn._fetch_listings_of_request_url(cs,
            ...
            non_ad_listings_with_location_filter = await self.tn._fetch_listings_of_request_url(cs,
    ...
    async def test_empty_query_shouldnt_return_any_listings(self):
        ...
        tqs = TweedehandsQuerySpecs(query=query)
        async with aiohttp.ClientSession() as cs:
            non_ad_listings = await self.tn._fetch_listings_of_request_url(cs, tqs.request_query_url)
